...
아래부터 해당 서비스에 대해 추가 또는 개선하고자 하는 기능에 대한 내용입니다.

1. 예약자 알림 메일 전송 API
   - nodemailer를 사용하여 마스터 계정의 이메일로 고객에게 안내 이메일 전송하는 기능
2. 식당 예약 시간을 점주가 가능한 시간대를 등록하고, 해당 시간대를 고객이 선택하여 예약하는 방식으로 변경하면 좋았겠다는 생각을 했습니다.
   - 예약 요청마다 시간을 검증하는 것보다 점주가 시간을 등록할때만 검증되고, 사용자는 등록된 시간대 id값만 사용하여 예약하게 되면 불필요한 시간 검증 없이 예약할 수 있지 않을까 생각합니다.
3. 이미지 처리
   - 사용자의 프로필 또는 가게의 이미지(메뉴나 건물, 실내 등) 업로드 및 이미지 파일 응답 등의 기능 추가가 있었으면 좋았겠다는 생각을 합니다.
   - sharp 라이브러리와 multer를 통해 이미지 퀄리티를 조정하고 데이터베이스에 byte로 저장하는 방법과 버퍼를 통해 응답하는 기능을 생각 했습니다.
4. throttling 설정으로 IP 기반 요청 횟수 제한
   - 요청 횟수 제한을 두어 무작위 대입 요청 방지가 필요하다고 생각합니다.